...
void setHoldWithHoldType(thermostatId, coolingSetPoint, heatingSetPoint, fanMode,
	...
	thermostatId = determine_tstat_id(thermostatId)
	traceEvent(settings.logFilter,"setholdWithHoldType>called with values holdType=$holdType, coolSP=${coolingSetPoint}, heatSP=${heatingSetPoint}, fanMode=${fanMode}, extraHoldParams=${extraHoldParams} for ${thermostatId}", settings.trace)
	...
	if (data.thermostatList[0]?.remoteSensors) {
		for (i in 0..data.thermostatList[0].remoteSensors.size() - 1) {
			...
			for (j in 0..data.thermostatList[0].remoteSensors[i].capability.size()-1) {
                ...
                //JMP 20181202 added
                //log.error "REMOTE SENSOR: ${data.thermostatList[0].remoteSensors[i].capability[j].type}"
                //log.error "REMOTE SENSOR: ${data.thermostatList[0].remoteSensors[i].name} - ${data.thermostatList[0].remoteSensors[i].inUse}"
                //JMP end added

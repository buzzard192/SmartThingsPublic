...
def ecobeeDeviceList() {
	...
	stats = stats + ems
	int tstatCount=stats.size()    
	traceEvent(settings.logFilter,"ecobeeDeviceList>device list: $stats", detailedNotif)
	def p = dynamicPage(name: "deviceList", title: "Select Your Thermostats (${tstatCount} found)", uninstall: true) {
		section(""){
			paragraph image: "${getCustomImagePath()}ecobee4.jpg", "Tap below to see the list of ecobee thermostats available in your ecobee account.\n\nIf you have disconnect issues with your ST account, select only 1 tstat and create 1 instance of MyEcobeeInit per tstat"

...
def get_APP_VERSION() {return "8.2.6"}
...
//JMP 20180918 changed from 5 minutes to 1
...
def rescheduleIfNeeded(evt) {
    ...
    //JMP 20180918 changed from 5 to 1
	...
	if (((state?.poll["last"]?:0) + (delay * 60000) < currentTime) && canSchedule()) {
		...
		try {        
            //JMP 20180918 changed from runEvery5Minutes to runEvery1Minute
			runEvery1Minute(setZoneSettings)
		} catch (e) {
...
private def adjust_vent_settings_in_zone(indiceSchedule) {
	...
	// JMP - 20180817
    // If system is idle, do not adjust vents, just finish the processing
    //test
    //traceEvent(settings.logFilter,"test forcing sleep", settings.detailedNotif, get_LOG_ERROR(), settings.detailedNotif)
    //thermostat?.asleep()
    
    if (operatingState.toUpperCase().contains("IDLE")) {
    	traceEvent(settings.logFilter,"adjust_vent_settings_in_zone>System is idle, not adjusting vents, exiting",
        	settings.detailedNotif, get_LOG_INFO(), settings.detailedNotif)
            //open_all_vents()
        return ventSwitchesOnSet
    }
    // JMP - 20180817 end of addition
...
private def setVentSwitchLevel(indiceRoom, ventSwitch, switchLevel=100) {
	...
	// JMP - 20180821
    // Don't attempt to adjust a vent that is at the desired setting
    if (switchLevel == getCurrentVentLevel(ventSwitch)) {
		//traceEvent(settings.logFilter,"${ventSwitch} level ${switchLevel} in room ${roomName} already set",settings.detailedNotif, get_LOG_WARN())
        return true
    }        
     
	try {
    	// JMP - 20180821
        // added to close/open vents slowly
        if (switchLevel == 100) {
        	ventSwitch.setLevel(99)
        } else if (switchLevel == 0) {
        	ventSwitch.setLevel(1)
        }
        // JMP end of added section
